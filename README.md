...
Pins are grouped with **Leaflet.markercluster**: on a day that includes a long inter-city transfer (e.g. Hakone, where the Tokyo departure point is ~80km from the rest of that day), the map has to zoom out far enough that the local stops would otherwise overlap pixel-for-pixel — including two activities at the exact same coordinates (the same hotel used to check in and, later, for dinner). Instead of silently stacking pins on top of each other, overlapping ones collapse into a numbered cluster bubble; click it (or zoom in) to split it apart.
